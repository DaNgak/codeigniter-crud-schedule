    ...
    <div class="col-lg-9">
        <div class="card shadow mb-4">
            ...
            <div class="card-body">
                ...
                <form action="<?= site_url('/dashboard/jadwal/update/' . esc($jadwal['id'])) ?>" method="post">
                    ...
                    <div class="form-group">
                        ...
                        <select name="periode_kuliah_id" id="periode_kuliah_id" class="form-control">
                            ...
                            <?php foreach ($periodeKuliah as $pk): ?>
                                <option value="<?= esc($pk['id']) ?>" <?= old('periode_kuliah_id', $jadwal['periode_kuliah_id']) == $pk['id'] ? 'selected' : '' ?>>
                    ...
                    <div class="form-group">
                        ...
                        <select name="program_studi_id" id="program_studi_id" class="form-control">
                            ...
                            <?php foreach ($programStudi as $ps): ?>
                                <option value="<?= esc($ps['id']) ?>" <?= old('program_studi_id', $jadwal['program_studi_id']) == $ps['id'] ? 'selected' : '' ?>>
                                    <?= esc($ps['nama']) ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="waktu_kuliah_id" id="waktu_kuliah_id" class="form-control">
                            ...
                            <?php foreach ($waktuKuliah as $wk): ?>
                                <option value="<?= esc($wk['id']) ?>" <?= old('waktu_kuliah_id', $jadwal['waktu_kuliah_id']) == $wk['id'] ? 'selected' : '' ?>>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        var oldOrValueProgramStudi = "<?= old('program_studi_id', $jadwal['program_studi_id']) ?>";
        var oldOrValueKelas = "<?= old('kelas_id', $jadwal['kelas_id']) ?>";
        var oldOrValueMataKuliah = "<?= old('mata_kuliah_id', $jadwal['mata_kuliah_id']) ?>";
        var oldOrValueRuangan = "<?= old('ruangan_id', $jadwal['ruangan_id']) ?>";
        var oldOrValueDosen = "<?= old('dosen_id', $jadwal['dosen_id']) ?>";

        // Set nilai lama untuk program_studi_id dan trigger perubahan jika ada nilai lama
        if (oldOrValueProgramStudi) {
            $('#program_studi_id').val(oldOrValueProgramStudi);
            $.ajax({
                url: "<?= site_url('/dashboard/jadwal/dropdown/getOptionsByProgramStudi') ?>/" + oldOrValueProgramStudi,
                type: "GET",
                dataType: "json",
                success: function (response) {
                    if (response.code === 200) {
                        ...
                        if (oldOrValueKelas) {
                            $('#kelas_id').val(oldOrValueKelas);
                        }
                        if (oldOrValueMataKuliah) {
                            $('#mata_kuliah_id').val(oldOrValueMataKuliah);
                        }
                        if (oldOrValueRuangan) {
                            $('#ruangan_id').val(oldOrValueRuangan);
                        }
                        if (oldOrValueDosen) {
                            $('#dosen_id').val(oldOrValueDosen);
                        }
                    } else {
                        Swal.fire({
                            title: 'Failed',
